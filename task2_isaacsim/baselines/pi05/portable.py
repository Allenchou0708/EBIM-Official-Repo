...
def validate_profile(path: Path) -> dict[str, Any]:
    ...
    if not isinstance(policy.get("train_expert_only"), bool):
        raise ValueError("profile.policy.train_expert_only must be boolean")
    return profile
...
def doctor(profile_path: Path) -> tuple[dict[str, Any], list[str]]:
    ...
    is_full = profile["policy"].get("train_expert_only") is False
    ...
    return {
        ...
        "mode": "full_finetune" if is_full else "expert_only_smoke",
        "gpu": gpu,
...
def _last_training_metrics(log_path: Path) -> dict[str, float | None]:
    text = (
        log_path.read_text(encoding="utf-8", errors="replace")
        if log_path.is_file()
        else ""
...
def command_train(args: argparse.Namespace) -> int:
    ...
    if unsuccessful and environment["mode"] == "full_finetune":
        print("FAIL: full_finetune refuses success=false episodes")
        return 2
    ...
    manifest["training_metrics"] = _last_training_metrics(log_path)
